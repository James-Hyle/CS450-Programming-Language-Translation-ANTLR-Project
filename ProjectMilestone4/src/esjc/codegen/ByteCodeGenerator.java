...
public class ByteCodeGenerator {
  ...
  public static ClassByteCodes generate(
       ...
       final TypeTable tt) throws Error {
  ...
  protected static class Visitor extends ASTVisitor {
    ...
    protected void generateConstructor(final String className) {
      ...
      this.mv.visitLocalVariable("this", "L" + className + ";", null, l0, l1, 0);
    ...
    public boolean visit(final Assignment node) {
      ...
      if (lhsNode instanceof ArrayAccess aa) {
      } else if (lhsNode instanceof FieldAccess fa) {
        fa.getExpression().accept(this);
        final String varName = fa.getName().getIdentifier();
        final Object lhsDecl = this.symbolMap.get(fa.getName());
        if (lhsDecl instanceof FieldDeclaration fd) {
          final String className = ((TypeDeclaration) fd.getParent()).getName().getIdentifier();
          this.mv.visitFieldInsn(Opcodes.PUTFIELD, className, varName, convertType(this.typeMap.get(fd)));
        }
      } else if (lhsNode instanceof SimpleName sn) {
        final Type t = this.typeMap.get(sn);
        final Object decl = this.symbolMap.get(sn);
        if (decl instanceof FieldDeclaration fd) {
          final String className = ((TypeDeclaration) fd.getParent()).getName().getIdentifier();
          this.mv.visitFieldInsn(Opcodes.PUTSTATIC, className,
                  sn.getIdentifier(), convertType(this.typeMap.get(fd)));
        } else if (t instanceof IntType || t instanceof BooleanType) {
          this.mv.visitVarInsn(Opcodes.ISTORE, this.localIndexMap.get(sn.getIdentifier()));
        } else {
          this.mv.visitVarInsn(Opcodes.ASTORE, this.localIndexMap.get(sn.getIdentifier()));
        }
      }
      return false;
    }
    ...
    public boolean visit(final MethodDeclaration node) {
      ...
      if (!(lastStatement instanceof ReturnStatement)) {
        if (methodType.first instanceof VoidType) {
          this.mv.visitInsn(Opcodes.RETURN);
        } else if (methodType.first instanceof IntType
                || methodType.first instanceof BooleanType) {
          this.mv.visitInsn(Opcodes.ICONST_0);

...
public class TypeChecker {
  ...
  protected static class Visitor extends ASTVisitor {
    ...
    public boolean visit(final Assignment node) {
      ...
      if (rhsType == null) {
        if (lhsType == this.tf.Int || lhsType == this.tf.Boolean || lhsType == this.tf.Void) {
          throw new Error(node,
                  "Cannot assign null to primitive type in \"" + node + "\"");
        }
        return false;
      }
    ...
    public boolean visit(final InfixExpression node) {
      ...
      boolean nullComparison = (lhsType == null || rhsType == null);
      boolean bothNonPrimitive = (lhsType == null || lhsType instanceof ClassType || lhsType instanceof ArrayType)
              && (rhsType == null || rhsType instanceof ClassType || rhsType instanceof ArrayType);

      if (nullComparison && bothNonPrimitive) {
    ...
    public boolean visit(final PrefixExpression node) {
      ...
      if ((op == PrefixExpression.Operator.PLUS)
          || (op == PrefixExpression.Operator.MINUS) || op == PrefixExpression.Operator.COMPLEMENT) {
        if (t != this.tf.Int) {
          throw new Error(node,
    ...
    public boolean visit(final ReturnStatement node) {
      ...
      if ((this.methodReturnType == this.tf.Void) && (e != null)) {
        ...
        throw new Error(node, "Expecting a return's expression in \"" + node
            + "\"");
      } else if ((this.methodReturnType != this.tf.Void)) {
        e.accept(this);
        if (getResult() != this.methodReturnType) {
   ...
   */
  public static
  TypeTable check(final TypeFactory tf, final CompilationUnit cu,  final SymbolTable symbolTable) throws Error {
    assert (tf != null) && (cu != null) && (symbolTable != null);

    final Visitor v = new Visitor(tf, symbolTable);
    cu.accept(v);
    final TypeTable result = new TypeTable(v.resultTypeMap, v.resultMethodTypeMap);
    v.dispose();
    return result;
  }
